...
SELECT location, 
	date, 
	CAST(replace(total_cases,'.0','') as int) AS total_cases, 
	CAST(replace(new_cases,'.0','') as int) AS new_cases, 
	CAST(replace(total_deaths,'.0','') as int) AS total_deaths,  
...
SELECT SUM(CONVERT(int, replace(new_cases,'.0',''))) AS total_new_cases, 
	SUM(CONVERT(int, replace(new_deaths,'.0',''))) AS total_new_deaths,
